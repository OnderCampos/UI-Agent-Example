...
- **Input** (`src/components/ui/input.tsx`) — Text inputs for editable contact and address fields.
...
The new membership view is a client page under the `(shop)` route group, so it inherits the shared `Header` and `Footer` via `src/app/(shop)/layout.tsx`. The page builds the form from small, focused pieces:
...
Future views that need to display read-only member information can reuse the same `ReadOnlyField`/`ReadOnlyInput` pattern or extract them into `src/components/features/membership`.
